...
function App() {
	...
	return (
		<>
			<div className="w-full h-screen flex justify-center items-center bg-very-dark-blue">
				<div className="max-w-[1143px] w-full min-h-[520px] md:h-fit h-dvh grid justify-center grid-cols-custom-desktop grid-rows-custom-desktop gap-[1.875rem]">
					<div className="bg-dark-blue cols-[1_/_2] row-[1_/_-1] rounded-[1.15rem] relative">
						<div className="bg-blue h-[350px] w-full flex flex-col gap-10 p-8 rounded-[1.15rem]">
							...
							<article>
								...
								<h1 className='text-[2.5rem] leading-[3rem] text-white font-light'>Jeremy Robson</h1>
							</article>
						</div>
									...
									: item.title.toLowerCase() === "study" ? "bg-study"
										: item.title.toLowerCase() === "social" ? "bg-social"
											: item.title.toLowerCase() === "exercise" ? "bg-exercise"
												: item.title.toLowerCase() === "self care" ? "bg-self-care" : "bg-play"} rounded-[1rem_1rem_1.25rem_1.25rem] relative w-[255px] h-[244px] flex flex-col justify-end overflow-hidden`}>
